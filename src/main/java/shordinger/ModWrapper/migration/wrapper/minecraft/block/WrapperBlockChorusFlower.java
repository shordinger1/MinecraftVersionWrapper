...
public class WrapperBlockChorusFlower extends WrapperBlock {

    public static final PropertyInteger AGE = PropertyInteger.create("age", 0, 5);

    protected WrapperBlockChorusFlower() {
        ...
        if (!this.canSurvive(worldIn, pos)) {
            ...
            if (worldIn.isAirBlock(blockpos) && blockpos.getY() < 256) {
                ...
                if (i < 5 && net.minecraftforge.common.ForgeHooks
                    ...
                    if (wrapperBlock == Blocks.END_STONE) {
                        flag = true;
                    } else if (wrapperBlock == Blocks.CHORUS_PLANT) {
                        int j = 1;

                        for (int k = 0; k < 4; ++k) {
                            WrapperBlock wrapperBlock1 = worldIn.getBlockState(pos.down(j + 1))
                                .getBlock();

                            if (wrapperBlock1 != Blocks.CHORUS_PLANT) {
                                if (wrapperBlock1 == Blocks.END_STONE) {
                                    flag1 = true;
                                }
    ...
    public boolean canSurvive(World worldIn, BlockPos pos) {
        ...
        if (wrapperBlock != Blocks.CHORUS_PLANT && wrapperBlock != Blocks.END_STONE) {
            if (iblockstate.getMaterial() == Material.AIR) {
                int i = 0;

                for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
                    IWrapperBlockState iblockstate1 = worldIn.getBlockState(pos.offset(enumfacing));
                    WrapperBlock wrapperBlock1 = iblockstate1.getBlock();
