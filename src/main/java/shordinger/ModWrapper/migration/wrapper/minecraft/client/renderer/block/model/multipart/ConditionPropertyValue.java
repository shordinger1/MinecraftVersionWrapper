...
public class ConditionPropertyValue implements ICondition {
    ...
    public Predicate<IWrapperBlockState> getPredicate(BlockStateContainer blockState) {
        ...
        if (iproperty == null) {
            ...
            if (list.isEmpty()) {
                ...
                Predicate<IWrapperBlockState> predicate;

                if (list.size() == 1) {
                    predicate = this.makePredicate(iproperty, s);
                } else {
                    predicate = Predicates
                        .or(Iterables.transform(list, new Function<String, Predicate<IWrapperBlockState>>() {

                            @Nullable
                            public Predicate<IWrapperBlockState> apply(@Nullable String p_apply_1_) {
    ...
    private Predicate<IWrapperBlockState> makePredicate(final IProperty<?> property, String valueIn) {
