...
public class WrapperBlockTripWire extends WrapperBlock {
        ...
        for (EnumFacing enumfacing : new EnumFacing[] { EnumFacing.SOUTH, EnumFacing.WEST }) {
            for (int i = 1; i < 42; ++i) {
                ...
                if (iblockstate.getBlock() == Blocks.TRIPWIRE_HOOK) {
                    if (iblockstate.getValue(WrapperBlockTripWireHook.FACING) == enumfacing.getOpposite()) {
    ...
    public static boolean isConnectedTo(IBlockAccess worldIn, BlockPos pos, IWrapperBlockState state,
        ...
        if (wrapperBlock == Blocks.TRIPWIRE_HOOK) {
            EnumFacing enumfacing = direction.getOpposite();
            return iblockstate.getValue(WrapperBlockTripWireHook.FACING) == enumfacing;
        } else {
            return wrapperBlock == Blocks.TRIPWIRE;
        }
    }
