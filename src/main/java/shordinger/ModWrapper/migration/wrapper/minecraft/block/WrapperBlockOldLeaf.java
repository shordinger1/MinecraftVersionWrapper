...
public class WrapperBlockOldLeaf extends WrapperBlockLeaves {
    ...
    protected void dropApple(World worldIn, BlockPos pos, IWrapperBlockState state, int chance) {
        if (state.getValue(VARIANT) == WrapperBlockPlanks.EnumType.OAK && worldIn.rand.nextInt(chance) == 0) {
            spawnAsEntity(worldIn, pos, new ItemStack(Items.APPLE));
        }
    }

    protected int getSaplingDropChance(IWrapperBlockState state) {
        return state.getValue(VARIANT) == WrapperBlockPlanks.EnumType.JUNGLE ? 40 : super.getSaplingDropChance(state);
    }
    ...
    public void getSubBlocks(CreativeTabs itemIn, NonNullList<ItemStack> items) {
        items.add(new ItemStack(this, 1, WrapperBlockPlanks.EnumType.OAK.getMetadata()));
        items.add(new ItemStack(this, 1, WrapperBlockPlanks.EnumType.SPRUCE.getMetadata()));
        items.add(new ItemStack(this, 1, WrapperBlockPlanks.EnumType.BIRCH.getMetadata()));
        items.add(new ItemStack(this, 1, WrapperBlockPlanks.EnumType.JUNGLE.getMetadata()));
    }
