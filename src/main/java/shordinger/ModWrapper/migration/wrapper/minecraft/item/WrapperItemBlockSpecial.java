...
public class WrapperItemBlockSpecial extends WrapperItem {

    private final Block block;

    public WrapperItemBlockSpecial(Block block) {
        this.block = block;
    }
        ...
        if (!itemstack.isEmpty() && player.canPlayerEdit(pos, facing, itemstack)
            ...
            if (!worldIn.setBlockState(pos, iblockstate1, 11)) {
                ...
                if (iblockstate1.getBlock() == this.block) {
                    WrapperItemBlock.setTileEntityNBT(worldIn, player, pos, itemstack);
