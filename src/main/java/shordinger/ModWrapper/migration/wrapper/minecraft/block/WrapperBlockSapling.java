...
public class WrapperBlockSapling extends WrapperBlockBush implements IGrowable {

    public static final PropertyEnum<WrapperBlockPlanks.EnumType> TYPE = PropertyEnum.<WrapperBlockPlanks.EnumType>create(
        "type",
    ...
    protected WrapperBlockSapling() {
        this.setDefaultState(
            this.blockState.getBaseState()
                .withProperty(TYPE, WrapperBlockPlanks.EnumType.OAK)
    ...
    public String getLocalizedName() {
        return I18n.translateToLocal(
            this.getUnlocalizedName() + "." + WrapperBlockPlanks.EnumType.OAK.getUnlocalizedName() + ".name");
    }
    ...
    public void grow(World worldIn, BlockPos pos, IWrapperBlockState state, Random rand) {
    ...
    public void generateTree(World worldIn, BlockPos pos, IWrapperBlockState state, Random rand) {
        ...
        switch ((WrapperBlockPlanks.EnumType) state.getValue(TYPE)) {
            case SPRUCE:
                label68:

                for (i = 0; i >= -1; --i) {
                    for (j = 0; j >= -1; --j) {
                        if (this.isTwoByTwoOfType(worldIn, pos, i, j, WrapperBlockPlanks.EnumType.SPRUCE)) {
            ...
            case BIRCH:
                ...
                break;
            case JUNGLE:
                IWrapperBlockState iblockstate = Blocks.LOG.getDefaultState()
                    .withProperty(WrapperBlockOldLog.VARIANT, WrapperBlockPlanks.EnumType.JUNGLE);
                IWrapperBlockState iblockstate1 = Blocks.LEAVES.getDefaultState()
                    .withProperty(WrapperBlockOldLeaf.VARIANT, WrapperBlockPlanks.EnumType.JUNGLE)
                    .withProperty(WrapperBlockLeaves.CHECK_DECAY, Boolean.valueOf(false));
                label82:

                for (i = 0; i >= -1; --i) {
                    for (j = 0; j >= -1; --j) {
                        if (this.isTwoByTwoOfType(worldIn, pos, i, j, WrapperBlockPlanks.EnumType.JUNGLE)) {
            ...
            case DARK_OAK:
                ...
                for (i = 0; i >= -1; --i) {
                    for (j = 0; j >= -1; --j) {
                        if (this.isTwoByTwoOfType(worldIn, pos, i, j, WrapperBlockPlanks.EnumType.DARK_OAK)) {
        ...
        IWrapperBlockState iblockstate2 = Blocks.AIR.getDefaultState();
    ...
    private boolean isTwoByTwoOfType(World worldIn, BlockPos pos, int p_181624_3_, int p_181624_4_,
        WrapperBlockPlanks.EnumType type) {
     ...
     */
    public boolean isTypeAt(World worldIn, BlockPos pos, WrapperBlockPlanks.EnumType type) {
    ...
    public int damageDropped(IWrapperBlockState state) {
        return ((WrapperBlockPlanks.EnumType) state.getValue(TYPE)).getMetadata();
    }
    ...
    public IWrapperBlockState getStateFromMeta(int meta) {
        return this.getDefaultState()
            .withProperty(TYPE, WrapperBlockPlanks.EnumType.byMetadata(meta & 7))
    ...
    public int getMetaFromState(IWrapperBlockState state) {
        int i = 0;
        i = i | ((WrapperBlockPlanks.EnumType) state.getValue(TYPE)).getMetadata();
