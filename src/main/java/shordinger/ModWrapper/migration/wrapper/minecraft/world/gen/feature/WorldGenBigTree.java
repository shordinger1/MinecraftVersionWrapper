...
public class WorldGenBigTree extends WorldGenAbstractTree {
    ...
    void crosSection(BlockPos pos, float p_181631_2_, IWrapperBlockState p_181631_3_) {
        ...
        for (int j = -i; j <= i; ++j) {
            for (int k = -i; k <= i; ++k) {
                if (Math.pow((double) Math.abs(j) + 0.5D, 2.0D) + Math.pow((double) Math.abs(k) + 0.5D, 2.0D)
                    ...
                    IWrapperBlockState state = this.world.getBlockState(blockpos);
    ...
    private boolean validTreeLocation() {
        ...
        net.minecraft.block.state.IWrapperBlockState state = this.world.getBlockState(down);
