...
public class WrapperBlockJukebox extends WrapperBlockContainer {
    ...
    public static void registerFixesJukebox(DataFixer fixer) {
        fixer.registerWalker(
            ...
            new ItemStackData(WrapperBlockJukebox.TileEntityJukebox.class, new String[] { "RecordItem" }));
    }

    protected WrapperBlockJukebox() {
    ...
    public void insertRecord(World worldIn, BlockPos pos, IWrapperBlockState state, ItemStack recordStack) {
        TileEntity tileentity = worldIn.getTileEntity(pos);

        if (tileentity instanceof WrapperBlockJukebox.TileEntityJukebox) {
            ((WrapperBlockJukebox.TileEntityJukebox) tileentity).setRecord(recordStack.copy());
            worldIn.setBlockState(pos, state.withProperty(HAS_RECORD, Boolean.valueOf(true)), 2);
        }
    }

    private void dropRecord(World worldIn, BlockPos pos, IWrapperBlockState state) {
        if (!worldIn.isRemote) {
            TileEntity tileentity = worldIn.getTileEntity(pos);

            if (tileentity instanceof WrapperBlockJukebox.TileEntityJukebox) {
                WrapperBlockJukebox.TileEntityJukebox blockjukebox$tileentityjukebox = (WrapperBlockJukebox.TileEntityJukebox) tileentity;
    ...
    public TileEntity createNewTileEntity(World worldIn, int meta) {
        return new WrapperBlockJukebox.TileEntityJukebox();
    }
    ...
    public int getComparatorInputOverride(IWrapperBlockState blockState, World worldIn, BlockPos pos) {
        ...
        if (tileentity instanceof WrapperBlockJukebox.TileEntityJukebox) {
            ItemStack itemstack = ((WrapperBlockJukebox.TileEntityJukebox) tileentity).getRecord();
