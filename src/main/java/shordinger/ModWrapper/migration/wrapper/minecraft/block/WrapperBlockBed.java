...
public class WrapperBlockBed extends WrapperBlockHorizontal implements ITileEntityProvider {

    public static final PropertyEnum<WrapperBlockBed.EnumPartType> PART = PropertyEnum.<WrapperBlockBed.EnumPartType>create(
        "part",
        WrapperBlockBed.EnumPartType.class);
    public static final PropertyBool OCCUPIED = PropertyBool.create("occupied");
    protected static final AxisAlignedBB BED_AABB = new AxisAlignedBB(0.0D, 0.0D, 0.0D, 1.0D, 0.5625D, 1.0D);

    public WrapperBlockBed() {
        super(Material.CLOTH);
        this.setDefaultState(
            this.blockState.getBaseState()
                .withProperty(PART, WrapperBlockBed.EnumPartType.FOOT)
    ...
    public MapColor getMapColor(IWrapperBlockState state, IBlockAccess worldIn, BlockPos pos) {
        if (state.getValue(PART) == WrapperBlockBed.EnumPartType.FOOT) {
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IWrapperBlockState state, EntityPlayer playerIn,
        ...
        if (worldIn.isRemote) {
            ...
            if (state.getValue(PART) != WrapperBlockBed.EnumPartType.HEAD) {
    ...
    public Item getItemDropped(IWrapperBlockState state, Random rand, int fortune) {
        return state.getValue(PART) == WrapperBlockBed.EnumPartType.FOOT ? Items.AIR : Items.BED;
    }
    ...
    public void onBlockHarvested(World worldIn, BlockPos pos, IWrapperBlockState state, EntityPlayer player) {
        if (player.capabilities.isCreativeMode && state.getValue(PART) == WrapperBlockBed.EnumPartType.FOOT) {
     ...
     */
    public void harvestBlock(World worldIn, EntityPlayer player, BlockPos pos, IWrapperBlockState state, TileEntity te,
        ItemStack stack) {
        if (state.getValue(PART) == WrapperBlockBed.EnumPartType.HEAD && te instanceof TileEntityBed) {
    ...
    public IWrapperBlockState getStateFromMeta(int meta) {
        ...
        return (meta & 8) > 0 ? this.getDefaultState()
            .withProperty(PART, WrapperBlockBed.EnumPartType.HEAD)
    ...
    public IWrapperBlockState getActualState(IWrapperBlockState state, IBlockAccess worldIn, BlockPos pos) {
        if (state.getValue(PART) == WrapperBlockBed.EnumPartType.FOOT) {
            IWrapperBlockState iblockstate = worldIn.getBlockState(pos.offset((EnumFacing) state.getValue(FACING)));
