...
public class WrapperBlockMushroom extends WrapperBlockBush implements IGrowable {
    ...
    protected WrapperBlockMushroom() {
    ...
    public boolean canBlockStay(World worldIn, BlockPos pos, IWrapperBlockState state) {
        if (pos.getY() >= 0 && pos.getY() < 256) {
            ...
            if (iblockstate.getBlock() == Blocks.MYCELIUM) {
                ...
                && iblockstate.getValue(WrapperBlockDirt.VARIANT) == WrapperBlockDirt.DirtType.PODZOL) {
                    return true;
                } else {
    ...
    public boolean generateBigMushroom(World worldIn, BlockPos pos, IWrapperBlockState state, Random rand) {
