...
public class WrapperItemMonsterPlacer extends WrapperItem {

    public WrapperItemMonsterPlacer() {
     ...
     */
    public static void applyItemEntityDataToEntity(World entityWorld, @Nullable EntityPlayer player, TempItemStack stack,
    ...
    public void getSubItems(CreativeTabs tab, NonNullList<TempItemStack> items) {
        if (this.isInCreativeTab(tab)) {
            for (EntityList.EntityEggInfo entitylist$entityegginfo : EntityList.ENTITY_EGGS.values()) {
                TempItemStack itemstack = new TempItemStack(this, 1);
     ...
     */
    public static void applyEntityIdToItemStack(TempItemStack stack, ResourceLocation entityId) {
     ...
     */
    @Nullable
    public static ResourceLocation getNamedIdFrom(TempItemStack stack) {
        NBTTagCompound nbttagcompound = stack.getTagCompound();
