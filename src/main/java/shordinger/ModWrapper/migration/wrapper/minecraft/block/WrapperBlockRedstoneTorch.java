...
public class WrapperBlockRedstoneTorch extends WrapperBlockTorch {

    private static final Map<World, List<WrapperBlockRedstoneTorch.Toggle>> toggles = new java.util.WeakHashMap<World, List<Toggle>>(); // FORGE
                                                                                                                                        // -
                                                                                                                                        // fix
                                                                                                                                        // vanilla
                                                                                                                                        // MC-101233
    ...
    private boolean isBurnedOut(World worldIn, BlockPos pos, boolean turnOff) {
        ...
        List<WrapperBlockRedstoneTorch.Toggle> list = (List) toggles.get(worldIn);

        if (turnOff) {
            list.add(new WrapperBlockRedstoneTorch.Toggle(pos, worldIn.getTotalWorldTime()));
        }

        int i = 0;

        for (int j = 0; j < list.size(); ++j) {
            WrapperBlockRedstoneTorch.Toggle blockredstonetorch$toggle = list.get(j);
    ...
    protected WrapperBlockRedstoneTorch(boolean isOn) {
        this.isOn = isOn;
    ...
    private boolean shouldBeOff(World worldIn, BlockPos pos, IWrapperBlockState state) {
