...
public class WrapperBlockFire extends WrapperBlock {
    ...
    private final Map<WrapperBlock, Integer> encouragements = Maps.<WrapperBlock, Integer>newIdentityHashMap();
    private final Map<WrapperBlock, Integer> flammabilities = Maps.<WrapperBlock, Integer>newIdentityHashMap();
    ...
    public void setFireInfo(WrapperBlock wrapperBlockIn, int encouragement, int flammability) {
        if (wrapperBlockIn == Blocks.AIR)
            throw new IllegalArgumentException("Tried to set air on fire... This is bad.");
        this.encouragements.put(wrapperBlockIn, Integer.valueOf(encouragement));
        this.flammabilities.put(wrapperBlockIn, Integer.valueOf(flammability));
    }
    ...
    public void updateTick(World worldIn, BlockPos pos, IWrapperBlockState state, Random rand) {
        if (worldIn.getGameRules()
            ...
            WrapperBlock wrapperBlock = worldIn.getBlockState(pos.down())
                .getBlock();
            boolean flag = wrapperBlock.isFireSource(worldIn, pos.down(), EnumFacing.UP);
    ...
    public int getFlammability(WrapperBlock wrapperBlockIn) {
        Integer integer = this.flammabilities.get(wrapperBlockIn);
        return integer == null ? 0 : integer.intValue();
    }

    @Deprecated // Use Block.getFireSpreadSpeed
    public int getEncouragement(WrapperBlock wrapperBlockIn) {
        Integer integer = this.encouragements.get(wrapperBlockIn);
        return integer == null ? 0 : integer.intValue();
    }
        ...
        if (random.nextInt(chance) < i) {
            ...
            if (iblockstate.getBlock() == Blocks.TNT) {
                Blocks.TNT.onBlockDestroyedByPlayer(
                    worldIn,
                    pos,
                    iblockstate.withProperty(WrapperBlockTNT.EXPLODE, Boolean.valueOf(true)));
            }
        }
    }
