...
import shordinger.ModWrapper.migration.wrapper.minecraft.util.datafix.DataFixer;
import shordinger.ModWrapper.migration.wrapper.minecraft.util.datafix.FixTypes;
import shordinger.ModWrapper.migration.wrapper.minecraft.util.datafix.walkers.BlockEntityTag;
import shordinger.ModWrapper.migration.wrapper.minecraft.util.datafix.walkers.EntityTag;
import shordinger.ModWrapper.migration.wrapper.minecraft.util.math.BlockPos;
import shordinger.ModWrapper.migration.wrapper.minecraftforge.common.capabilities.CapabilityDispatcher;
import shordinger.ModWrapper.migration.wrapper.minecraftforge.registries.IRegistryDelegate;

public final class TempItemStack {

    public static final TempItemStack EMPTY = new TempItemStack((WrapperItem) null);
    ...
    private final WrapperItem wrapperItem;
    /**
    ...
    private IRegistryDelegate<WrapperItem> delegate;
    private CapabilityDispatcher capabilities;
    private NBTTagCompound capNBT;

    public ItemStack getItemStack() {
        return new ItemStack(getItem(), stackSize, getMetadata());
    }

    public static TempItemStack getTempItemStack(ItemStack stack) {
        return new TempItemStack((WrapperItem) stack.getItem(), stack.stackSize, stack.getItem().getMetadata(0), stack.getTagCompound());
    }


    public TempItemStack(Block blockIn) {
        this(blockIn, 1);
    }

    public TempItemStack(Block blockIn, int amount) {
        this(blockIn, amount, 0);
    }

    public TempItemStack(Block blockIn, int amount, int meta) {
        this(WrapperItem.getItemFromBlock(blockIn), amount, meta);
    }

    public TempItemStack(WrapperItem wrapperItemIn) {
        this(wrapperItemIn, 1);
    }

    public TempItemStack(WrapperItem wrapperItemIn, int amount) {
        this(wrapperItemIn, amount, 0);
    }

    public TempItemStack(WrapperItem wrapperItemIn, int amount, int meta) {
        this(wrapperItemIn, amount, meta, null);
    }

    public TempItemStack(WrapperItem wrapperItemIn, int amount, int meta, @Nullable NBTTagCompound capNBT) {
        this.capNBT = capNBT;
        this.wrapperItem = wrapperItemIn;
    ...
    public TempItemStack(NBTTagCompound compound) {
        this.capNBT = compound.hasKey("ForgeCaps") ? compound.getCompoundTag("ForgeCaps") : null;
        this.wrapperItem = compound.hasKey("id", 8) ? WrapperItem.getByNameOrId(compound.getString("id")) : WrapperItems.AIR; // Forge fix
        // tons of
        ...
        if (compound.hasKey("tag", 10)) {
            ...
            if (this.wrapperItem != null) {
                this.wrapperItem.updateItemStackNBT(compound);
            }
        }
    ...
    public boolean isEmpty() {
        if (this == EMPTY) {
            return true;
        } else if (this.getItemRaw() != null && this.getItemRaw() != WrapperItems.AIR) {
            if (this.stackSize <= 0) {
                return true;
            } else {
     ...
     */
    public TempItemStack splitStack(int amount) {
        int i = Math.min(amount, this.stackSize);
        TempItemStack itemstack = this.copy();
     ...
     */
    public WrapperItem getItem() {
        return this.isEmpty || this.delegate == null ? WrapperItems.AIR : this.delegate.get();
    }
    ...
    public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand,
        ...
        if (!worldIn.isRemote) return net.minecraftforge.common.ForgeHooks
            .onPlaceItemIntoWorld(getItemStack(), playerIn, worldIn, pos.getX(), pos.getY(), pos.getZ(), side, hitX, hitY, hitZ, hand);
        EnumActionResult enumactionresult = this.getItem()
            .onItemUse(playerIn, worldIn, pos, hand, side, hitX, hitY, hitZ);

        if (enumactionresult == EnumActionResult.SUCCESS) {
            playerIn.addStat(StatList.getObjectUseStats(this.wrapperItem));
        }
    ...
    public float getDestroySpeed(IWrapperBlockState blockIn) {
        return this.getItem()
     ...
     */
    public ActionResult<TempItemStack> useItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand hand) {
        return this.getItem()
     ...
     */
    public TempItemStack onItemUseFinish(World worldIn, EntityLivingBase entityLiving) {
        return this.getItem()
    ...
    public NBTTagCompound writeToNBT(NBTTagCompound nbt) {
        ResourceLocation resourcelocation = WrapperItem.REGISTRY.getNameForObject(this.wrapperItem);
    ...
    public boolean isItemStackDamageable() {
        if (this.isEmpty) {
            return false;
        } else if (this.wrapperItem.getMaxDamage(this) <= 0) {
            return false;
        } else {
            ...
            if (this.isItemStackDamageable()) {
                if (this.attemptDamageItem(
                    ...
                    if (entityIn instanceof EntityPlayer) {
                        ...
                        entityplayer.addStat(StatList.getObjectBreakStats(this.wrapperItem));
                    }
    ...
    public void hitEntity(EntityLivingBase entityIn, EntityPlayer playerIn) {
        boolean flag = this.wrapperItem.hitEntity(this, entityIn, playerIn);
     ...
     */
    public void onBlockDestroyed(World worldIn, IWrapperBlockState blockIn, BlockPos pos, EntityPlayer playerIn) {
     ...
     */
    public TempItemStack copy() {
        TempItemStack itemstack = new TempItemStack(
            this.wrapperItem,
            this.stackSize,
            this.itemDamage,
    ...
    public static boolean areItemStackTagsEqual(TempItemStack stackA, TempItemStack stackB) {
     ...
     */
    public static boolean areItemStacksEqual(TempItemStack stackA, TempItemStack stackB) {
        if (stackA.isEmpty() && stackB.isEmpty()) {
            return true;
        } else {
     ...
     */
    private boolean isItemStackEqual(TempItemStack other) {
     ...
     */
    public static boolean areItemsEqual(TempItemStack stackA, TempItemStack stackB) {
        if (stackA == stackB) {
            return true;
        } else {
    ...
    public static boolean areItemsEqualIgnoreDurability(TempItemStack stackA, TempItemStack stackB) {
        if (stackA == stackB) {
            return true;
        } else {
     ...
     * equal
     */
    public boolean isItemEqual(TempItemStack other) {
        return !other.isEmpty() && this.wrapperItem == other.wrapperItem && this.itemDamage == other.itemDamage;
    }

    public boolean isItemEqualIgnoreDurability(TempItemStack stack) {
        if (!this.isItemStackDamageable()) {
            return this.isItemEqual(stack);
        } else {
            return !stack.isEmpty() && this.wrapperItem == stack.wrapperItem;
        }
    }
    ...
    public void updateAnimation(World worldIn, Entity entityIn, int inventorySlot, boolean isCurrentItem) {
        ...
        if (this.wrapperItem != null) {
            this.wrapperItem.onUpdate(this, worldIn, entityIn, inventorySlot, isCurrentItem);
        }
    }

    public void onCrafting(World worldIn, EntityPlayer playerIn, int amount) {
        playerIn.addStat(StatList.getCraftStats(this.wrapperItem), amount);
        this.getItem()
    ...
    public TempItemStack setTranslatableName(String p_190924_1_) {
        this.getOrCreateSubCompound("display")
            .setString("LocName", p_190924_1_);
        return this;
    }

    public TempItemStack setStackDisplayName(String displayName) {
        this.getOrCreateSubCompound("display")
    ...
    public List<String> getTooltip(@Nullable EntityPlayer playerIn, ITooltipFlag advanced) {
        ...
        if (advanced.isAdvanced()) {
            ...
            int i = WrapperItem.getIdFromItem(this.wrapperItem);
        ...
        } else if (!this.hasDisplayName() && this.wrapperItem == WrapperItems.FILLED_MAP) {
            s = s + " #" + this.itemDamage;
        }
        ...
        for (EntityEquipmentSlot entityequipmentslot : EntityEquipmentSlot.values()) {
            ...
            if (!multimap.isEmpty() && (i1 & 2) == 0) {
                ...
                for (Entry<String, AttributeModifier> entry : multimap.entries()) {
                    ...
                    if (playerIn != null) {
                        if (attributemodifier.getID() == WrapperItem.ATTACK_DAMAGE_MODIFIER) {
                            ...
                            flag = true;
                        } else if (attributemodifier.getID() == WrapperItem.ATTACK_SPEED_MODIFIER) {
        ...
        if (advanced.isAdvanced()) {
            ...
            list.add(
                TextFormatting.DARK_GRAY + ((ResourceLocation) WrapperItem.REGISTRY.getNameForObject(this.wrapperItem)).toString());
     ...
     * @see WrapperItem#canItemEditBlocks
     */
    ...
    public void deserializeNBT(NBTTagCompound nbt) {
        ...
        final TempItemStack tempItemStack = new TempItemStack(nbt);
        this.stackTagCompound = tempItemStack.stackTagCompound;
        this.capNBT = tempItemStack.capNBT;
    }
    ...
    public boolean areCapsCompatible(TempItemStack other) {
        if (this.capabilities == null) {
    ...
    private void forgeInit() {
        WrapperItem wrapperItem = getItemRaw();
        if (wrapperItem != null) {
            this.delegate = wrapperItem.delegate;
            net.minecraftforge.common.capabilities.ICapabilityProvider provider = wrapperItem
     ...
     */
    @Nullable
    private WrapperItem getItemRaw() {
     ...
     */
    public static boolean areItemStacksEqualUsingNBTShareTag(TempItemStack stackA, TempItemStack stackB) {
     ...
     * Only used for comparing itemStacks that were transferred from server to client using Item.getNBTShareTag.
     */
    private boolean isItemStackEqualUsingNBTShareTag(TempItemStack other) {
     ...
     * Only used for comparing itemStacks that were transferred from server to client using Item.getNBTShareTag.
     */
    public static boolean areItemStackShareTagsEqual(TempItemStack stackA, TempItemStack stackB) {
