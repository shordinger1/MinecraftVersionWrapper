...
public class WrapperBlockDragonEgg extends WrapperBlock {
    ...
    private void checkFall(World worldIn, BlockPos pos) {
        if (worldIn.isAirBlock(pos.down()) && WrapperBlockFalling.canFallThrough(worldIn.getBlockState(pos.down()))
            && pos.getY() >= 0) {
            int i = 32;

            if (!WrapperBlockFalling.fallInstantly
                ...
                for (blockpos = pos; worldIn.isAirBlock(blockpos)
                    && WrapperBlockFalling.canFallThrough(worldIn.getBlockState(blockpos))
