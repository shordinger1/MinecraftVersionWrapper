...
public class WrapperBlockPressurePlate extends WrapperBlockBasePressurePlate {

    public static final PropertyBool POWERED = PropertyBool.create("powered");
    private final WrapperBlockPressurePlate.Sensitivity sensitivity;

    protected WrapperBlockPressurePlate(Material materialIn, WrapperBlockPressurePlate.Sensitivity sensitivityIn) {
        super(materialIn);
    ...
    protected int getRedstoneStrength(IWrapperBlockState state) {
        return ((Boolean) state.getValue(POWERED)).booleanValue() ? 15 : 0;
    }

    protected IWrapperBlockState setRedstoneStrength(IWrapperBlockState state, int strength) {
        return state.withProperty(POWERED, Boolean.valueOf(strength > 0));
    }

    protected void playClickOnSound(World worldIn, BlockPos color) {
        if (this.blockWrapperMaterial == Material.WOOD) {
            worldIn.playSound(
                (EntityPlayer) null,
                color,
