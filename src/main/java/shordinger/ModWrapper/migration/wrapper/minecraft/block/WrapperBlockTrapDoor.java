...
public class WrapperBlockTrapDoor extends WrapperBlock {

    public static final PropertyDirection FACING = WrapperBlockHorizontal.FACING;
    public static final PropertyBool OPEN = PropertyBool.create("open");
    public static final PropertyEnum<WrapperBlockTrapDoor.DoorHalf> HALF = PropertyEnum.<WrapperBlockTrapDoor.DoorHalf>create(
        "half",
        WrapperBlockTrapDoor.DoorHalf.class);
    ...
    protected WrapperBlockTrapDoor(Material materialIn) {
        super(materialIn);
        this.setDefaultState(
            this.blockState.getBaseState()
                .withProperty(FACING, EnumFacing.NORTH)
                .withProperty(OPEN, Boolean.valueOf(false))
                .withProperty(HALF, WrapperBlockTrapDoor.DoorHalf.BOTTOM));
    ...
    public AxisAlignedBB getBoundingBox(IWrapperBlockState state, IBlockAccess source, BlockPos pos) {
        ...
        } else if (state.getValue(HALF) == WrapperBlockTrapDoor.DoorHalf.TOP) {
    ...
    protected void playSound(@Nullable EntityPlayer player, World worldIn, BlockPos pos, boolean p_185731_4_) {
        if (p_185731_4_) {
            int i = this.blockWrapperMaterial == Material.IRON ? 1037 : 1007;
            worldIn.playEvent(player, i, pos, 0);
        } else {
            int j = this.blockWrapperMaterial == Material.IRON ? 1036 : 1013;
    ...
    public void neighborChanged(IWrapperBlockState state, World worldIn, BlockPos pos, WrapperBlock wrapperBlockIn,
        ...
        if (!worldIn.isRemote) {
            ...
            if (flag || wrapperBlockIn.getDefaultState()
                .canProvidePower()) {
    ...
    public IWrapperBlockState getStateForPlacement(World worldIn, BlockPos pos, EnumFacing facing, float hitX,
        ...
        if (facing.getAxis()
            ...
            iblockstate = iblockstate.withProperty(
                HALF,
                hitY > 0.5F ? WrapperBlockTrapDoor.DoorHalf.TOP : WrapperBlockTrapDoor.DoorHalf.BOTTOM);
        } else {
            iblockstate = iblockstate.withProperty(
                FACING,
            ...
            iblockstate = iblockstate.withProperty(
                HALF,
                facing == EnumFacing.UP ? WrapperBlockTrapDoor.DoorHalf.BOTTOM : WrapperBlockTrapDoor.DoorHalf.TOP);
        }
    ...
    public IWrapperBlockState getStateFromMeta(int meta) {
        return this.getDefaultState()
            ...
            .withProperty(
                HALF,
                (meta & 8) == 0 ? WrapperBlockTrapDoor.DoorHalf.BOTTOM : WrapperBlockTrapDoor.DoorHalf.TOP);
    }
    ...
    public int getMetaFromState(IWrapperBlockState state) {
        ...
        if (state.getValue(HALF) == WrapperBlockTrapDoor.DoorHalf.TOP) {
            i |= 8;
        }
    ...
    public BlockFaceShape getBlockFaceShape(IBlockAccess worldIn, IWrapperBlockState state, BlockPos pos,
        EnumFacing face) {
        return (face == EnumFacing.UP && state.getValue(HALF) == WrapperBlockTrapDoor.DoorHalf.TOP
            || face == EnumFacing.DOWN && state.getValue(HALF) == WrapperBlockTrapDoor.DoorHalf.BOTTOM)
    ...
    public boolean isLadder(IWrapperBlockState state, IBlockAccess world, BlockPos pos, EntityLivingBase entity) {
        if (state.getValue(OPEN)) {
            IWrapperBlockState down = world.getBlockState(pos.down());
            if (down.getBlock() == net.minecraft.init.Blocks.LADDER)
                return down.getValue(WrapperBlockLadder.FACING) == state.getValue(FACING);
        }
        return false;
    }
