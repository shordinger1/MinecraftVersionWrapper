...
public class CommandClone extends CommandBase {
    ...
    public void execute(MinecraftServer server, IWrapperCommandSender sender, String[] args) throws CommandException {
        if (args.length < 9) {
            ...
            if (i > 32768) {
                ...
                boolean flag = false;
                Block block = null;
                Predicate<IWrapperBlockState> predicate = null;
                            ...
                            for (int j = structureboundingbox.minZ; j <= structureboundingbox.maxZ; ++j) {
                                for (int k = structureboundingbox.minY; k <= structureboundingbox.maxY; ++k) {
                                    for (int l = structureboundingbox.minX; l <= structureboundingbox.maxX; ++l) {
                                        ...
                                        IWrapperBlockState iblockstate = world.getBlockState(blockpos4);
    ...
    static class StaticCloneData {
        ...
        public StaticCloneData(BlockPos posIn, IWrapperBlockState stateIn, NBTTagCompound compoundIn) {
            this.pos = posIn;
