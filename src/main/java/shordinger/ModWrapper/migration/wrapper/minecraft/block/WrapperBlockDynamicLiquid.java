...
public class WrapperBlockDynamicLiquid extends WrapperBlockLiquid {

    int adjacentSourceBlocks;

    protected WrapperBlockDynamicLiquid(Material materialIn) {
        super(materialIn);
    }

    private void placeStaticBlock(World worldIn, BlockPos pos, IWrapperBlockState currentState) {
        worldIn.setBlockState(
            pos,
            getStaticBlock(this.blockWrapperMaterial).getDefaultState()
    ...
    public void updateTick(World worldIn, BlockPos pos, IWrapperBlockState state, Random rand) {
        ...
        if (this.blockWrapperMaterial == Material.LAVA && !worldIn.provider.doesWaterVaporize()) {
            j = 2;
        }
        ...
        if (i > 0) {
            ...
            if (this.adjacentSourceBlocks >= 2 && net.minecraftforge.event.ForgeEventFactory
                .canCreateFluidSource(worldIn, pos, state, this.blockWrapperMaterial == Material.WATER)) {
                IWrapperBlockState iblockstate = worldIn.getBlockState(pos.down());

                if (iblockstate.getMaterial()
                    .isSolid()) {
                    i1 = 0;
                } else if (iblockstate.getMaterial() == this.blockWrapperMaterial
                    && ((Integer) iblockstate.getValue(LEVEL)).intValue() == 0) {
                        i1 = 0;
                    }
            }

            if (this.blockWrapperMaterial == Material.LAVA && i < 8 && i1 < 8 && i1 > i && rand.nextInt(4) != 0) {
                k *= 4;
            }
        ...
        if (this.canFlowInto(worldIn, pos.down(), iblockstate1)) {
            if (this.blockWrapperMaterial == Material.LAVA && worldIn.getBlockState(pos.down())
    ...
    private void tryFlowInto(World worldIn, BlockPos pos, IWrapperBlockState state, int level) {
    ...
    private int getSlopeDistance(World worldIn, BlockPos pos, int distance, EnumFacing calculateFlowCost) {
        ...
        for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
            if (enumfacing != calculateFlowCost) {
                ...
                if (!this.isBlocked(worldIn, blockpos, iblockstate) && (iblockstate.getMaterial() != this.blockWrapperMaterial
                    || ((Integer) iblockstate.getValue(LEVEL)).intValue() > 0)) {
    ...
    private int getSlopeFindDistance(World worldIn) {
        return this.blockWrapperMaterial == Material.LAVA && !worldIn.provider.doesWaterVaporize() ? 2 : 4;
    }
    ...
    private boolean isBlocked(World worldIn, BlockPos pos, IWrapperBlockState state) {
        WrapperBlock wrapperBlock = state.getBlock(); // Forge: state must be valid for position
        Material mat = state.getMaterial();

        if (!(wrapperBlock instanceof WrapperBlockDoor) && wrapperBlock != Blocks.STANDING_SIGN
            && wrapperBlock != Blocks.LADDER
            && wrapperBlock != Blocks.REEDS) {
    ...
    private boolean canFlowInto(World worldIn, BlockPos pos, IWrapperBlockState state) {
        Material material = state.getMaterial();
        return material != this.blockWrapperMaterial && material != Material.LAVA && !this.isBlocked(worldIn, pos, state);
    }
