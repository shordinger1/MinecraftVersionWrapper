...
public enum WrapperRotation {
    ...
    private WrapperRotation(String nameIn) {
        this.name = nameIn;
    }

    public WrapperRotation add(WrapperRotation wrapperRotation) {
        switch (wrapperRotation) {
            case CLOCKWISE_180 -> {
                return switch (this) {
    ...
    static {
        ...
        for (WrapperRotation wrapperRotation : values()) {
            rotationNames[i++] = wrapperRotation.name;
        }
    }
}
