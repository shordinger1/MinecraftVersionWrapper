...
import shordinger.ModWrapper.migration.wrapper.minecraft.advancements.AdvancementManager;
import shordinger.ModWrapper.migration.wrapper.minecraft.advancements.FunctionManager;
import shordinger.ModWrapper.migration.wrapper.minecraft.block.state.IWrapperBlockState;
import shordinger.ModWrapper.migration.wrapper.minecraft.pathfinding.PathWorldListener;
...
import shordinger.ModWrapper.migration.wrapper.minecraft.world.border.WorldBorder;
import shordinger.ModWrapper.migration.wrapper.minecraft.world.chunk.WrapperChunk;
import shordinger.ModWrapper.migration.wrapper.minecraft.world.storage.loot.LootTableManager;
...
import shordinger.ModWrapper.migration.wrapper.minecraftforge.common.util.WorldCapabilityData;

public abstract class WrapperWorld extends World implements IWrapperBlockAccess, ICapabilityProvider {
    ...
    protected WrapperWorld(ISaveHandler saveHandlerIn, WorldInfo info, WorldProvider providerIn, Profiler profilerIn,
        boolean client) {
        super(saveHandlerIn, null, providerIn, null, profilerIn);
    ...
    public Biome getBiomeForCoordsBody(final BlockPos pos) {
        if (this.isBlockLoaded(pos)) {
            WrapperChunk wrapperChunk = this.getChunkFromBlockCoords(pos);

            try {
                return wrapperChunk.getBiome(pos, this.provider.getBiomeProvider());
    ...
    public IWrapperBlockState getGroundAboveSeaLevel(BlockPos pos) {
        BlockPos blockpos;
    ...
    public WrapperChunk getChunkFromBlockCoords(BlockPos pos) {
     ...
     */
    public WrapperChunk getChunkFromChunkCoords(int chunkX, int chunkZ) {
     ...
     */
    public boolean setBlockState(BlockPos pos, IWrapperBlockState newState, int flags) {
        if (this.isOutsideBuildHeight(pos)) {
            ...
            IWrapperBlockState oldState = getBlockState(pos);
            int oldLight = oldState.getLightValue(this, pos);
            int oldOpacity = oldState.getLightOpacity(this, pos);

            IWrapperBlockState iblockstate = wrapperChunk.setBlockState(pos, newState);
            ...
            if (iblockstate == null) {
                ...
                {
                    this.markAndNotifyBlock(pos, wrapperChunk, iblockstate, newState, flags);
                }
                return true;
            }
        }
    ...
    // Split off from original setBlockState(BlockPos, IWrapperBlockState, int) method in order to directly send client
    // and physic updates
    public void markAndNotifyBlock(BlockPos pos, @Nullable WrapperChunk wrapperChunk, IWrapperBlockState iblockstate,
        IWrapperBlockState newState, int flags) {
        Block block = newState.getBlock();
        {
            {
                if ((flags & 2) != 0 && (!this.isRemote || (flags & 4) == 0)
                    && (wrapperChunk == null || wrapperChunk.isPopulated())) {
     ...
     */
    public boolean setBlockState(BlockPos pos, IWrapperBlockState state) {
     ...
     */
    public void notifyBlockUpdate(BlockPos pos, IWrapperBlockState oldState, IWrapperBlockState newState, int flags) {
        ...
        if (pos.getY() >= this.getSeaLevel()) {
            ...
            if (!this.canSeeSky(blockpos)) {
                ...
                for (BlockPos blockpos1 = blockpos.down(); blockpos1.getY()
                    ...
                    IWrapperBlockState iblockstate = this.getBlockState(blockpos1);
    ...
    public int getLight(BlockPos pos, boolean checkNeighbors) {
        if (pos.getX() >= -30000000 && pos.getZ() >= -30000000 && pos.getX() < 30000000 && pos.getZ() < 30000000) {
            if (checkNeighbors && this.getBlockState(pos)
                ...
                WrapperChunk wrapperChunk = this.getChunkFromBlockCoords(pos);
                return wrapperChunk.getLightSubtracted(pos, this.skylightSubtracted);
            }
        } else {
            return 15;
        }
    ...
    public int getChunksLowestHorizon(int x, int z) {
        if (x >= -30000000 && z >= -30000000 && x < 30000000 && z < 30000000) {
            if (!this.isChunkLoaded(x >> 4, z >> 4, true)) {
                ...
                WrapperChunk wrapperChunk = this.getChunkFromChunkCoords(x >> 4, z >> 4);
                return wrapperChunk.getLowestHeight();
            }
        } else {
        ...
        if (!this.provider.hasSkyLight() && type == EnumSkyBlock.SKY) {
            ...
            if (!this.isValid(pos)) {
                ...
                .useNeighborBrightness()) {
                    ...
                    WrapperChunk wrapperChunk = this.getChunkFromBlockCoords(pos);
                    return wrapperChunk.getLightFor(type, pos);
                }
        }
    }
    ...
    public void setLightFor(EnumSkyBlock type, BlockPos pos, int lightValue) {
        if (this.isValid(pos)) {
            if (this.isBlockLoaded(pos)) {
                WrapperChunk wrapperChunk = this.getChunkFromBlockCoords(pos);
                wrapperChunk.setLightFor(type, pos, lightValue);
    ...
    public IWrapperBlockState getBlockState(BlockPos pos) {
        if (this.isOutsideBuildHeight(pos)) {
            ...
            WrapperChunk wrapperChunk = this.getChunkFromBlockCoords(pos);
            return wrapperChunk.getBlockState(pos);
        }
    }
        ...
        if (!Double.isNaN(vec31.x) && !Double.isNaN(vec31.y) && !Double.isNaN(vec31.z)) {
            if (!Double.isNaN(vec32.x) && !Double.isNaN(vec32.y) && !Double.isNaN(vec32.z)) {
                ...
                while (k1-- >= 0) {
                    ...
                    IWrapperBlockState iblockstate1 = this.getBlockState(blockpos);
     ...
     *                 IWrapperBlockState#addCollisionBoxToList}. May be null, in which case the world border is
     *                 skipped.
    ...
    private boolean getCollisionBoxes(@Nullable Entity entityIn, AxisAlignedBB aabb, boolean p_191504_3_,
        ...
        IWrapperBlockState iblockstate = Blocks.STONE.getDefaultState();
                    ...
                    if ((!flag2 || !flag3) && this.isBlockLoaded(blockpos$pooledmutableblockpos.setPos(k1, 64, l1))) {
                        for (int i2 = k; i2 < l; ++i2) {
                            if (!flag2 && !flag3 || i2 != l - 1) {
                                ...
                                IWrapperBlockState iblockstate1;
    ...
    public BlockPos getTopSolidOrLiquidBlock(BlockPos pos) {
        ...
        for (blockpos = new BlockPos(pos.getX(), wrapperChunk.getTopFilledSegment() + 16, pos.getZ()); blockpos.getY()
            >= 0; blockpos = blockpos1) {
            blockpos1 = blockpos.down();
            IWrapperBlockState state = wrapperChunk.getBlockState(blockpos1);
        ...
        while (iterator.hasNext()) {
            ...
            if (tileentity.isInvalid()) {
                ...
                if (this.isBlockLoaded(tileentity.getPos())) {
                    ...
                    // desyned
                    WrapperChunk wrapperChunk = this.getChunkFromBlockCoords(tileentity.getPos());
                    if (wrapperChunk
                        .getTileEntity(tileentity.getPos(), net.minecraft.world.chunk.Chunk.EnumCreateEntityType.CHECK)
                        == tileentity) wrapperChunk.removeTileEntity(tileentity.getPos());
                }
            }
        }
            ...
            for (int j1 = 0; j1 < this.addedTileEntityList.size(); ++j1) {
                ...
                if (!tileentity1.isInvalid()) {
                    ...
                    if (this.isBlockLoaded(tileentity1.getPos())) {
                        WrapperChunk wrapperChunk = this.getChunkFromBlockCoords(tileentity1.getPos());
                        IWrapperBlockState iblockstate = wrapperChunk.getBlockState(tileentity1.getPos());
                        wrapperChunk.addTileEntity(tileentity1.getPos(), tileentity1);
    ...
    public boolean addTileEntity(TileEntity tile) {
        ...
        if (this.isRemote) {
            ...
            IWrapperBlockState iblockstate1 = this.getBlockState(blockpos1);
        ...
        for (int l3 = j2; l3 < k2; ++l3) {
            for (int i4 = l2; i4 < i3; ++i4) {
                for (int j4 = j3; j4 < k3; ++j4) {
                    IWrapperBlockState iblockstate1 = this
            ...
            for (int l3 = j2; l3 < k2; ++l3) {
                for (int i4 = l2; i4 < i3; ++i4) {
                    for (int j4 = j3; j4 < k3; ++j4) {
                        ...
                        IWrapperBlockState iblockstate1 = this.getBlockState(blockpos$pooledmutableblockpos);
        ...
        if (this.isOutsideBuildHeight(pos)) {
            ...
            if (tileentity2 == null) {
                tileentity2 = this.getChunkFromBlockCoords(pos)
                    .getTileEntity(pos, WrapperChunk.EnumCreateEntityType.IMMEDIATE);
            }
        ...
        if (!this.isOutsideBuildHeight(pos)) {
            if (tileEntityIn != null && !tileEntityIn.isInvalid()) {
                if (this.processingLoadedTiles) {
                    ...
                    WrapperChunk wrapperChunk = this.getChunkFromBlockCoords(pos);
                    if (wrapperChunk != null) wrapperChunk.addTileEntity(pos, tileEntityIn);
    ...
    public boolean isBlockNormalCube(BlockPos pos, boolean _default) {
        if (this.isOutsideBuildHeight(pos)) {
            ...
            WrapperChunk wrapperChunk1 = this.chunkProvider.getLoadedChunk(pos.getX() >> 4, pos.getZ() >> 4);

            if (wrapperChunk1 != null && !wrapperChunk1.isEmpty()) {
                IWrapperBlockState iblockstate1 = this.getBlockState(pos);
    ...
    @SideOnly(Side.CLIENT)
    protected void playMoodSoundAndCheckLight(int x, int z, WrapperChunk wrapperChunkIn) {
        wrapperChunkIn.enqueueRelightChecks();
    }

    protected void updateBlocks() {}

    public void immediateBlockTick(BlockPos pos, IWrapperBlockState state, Random random) {
                        ...
                        if (i5 > 0) {
                            ...
                            if (k5 + l5 + i6 < updateRange) {
                                ...
                                for (EnumFacing enumfacing : EnumFacing.values()) {
                                    ...
                                    IWrapperBlockState bs = this.getBlockState(blockpos$pooledmutableblockpos);
                                    int i7 = Math.max(
                                        1,
                                        bs.getBlock()
    ...
    @Nullable
    public List<NextTickListEntry> getPendingBlockUpdates(WrapperChunk wrapperChunkIn, boolean remove) {
        return null;
    }
    ...
    public CrashReportCategory addWorldInfoToCrashReport(CrashReport report) {
        ...
        crashreportcategory3.addDetail("All players", new ICrashReportDetail<String>() {
            ...
            public String call() {
                return WrapperWorld.this.playerEntities.size() + " total; " + WrapperWorld.this.playerEntities;
            }
        });
        crashreportcategory3.addDetail("Chunk stats", new ICrashReportDetail<String>() {

            public String call() {
                return WrapperWorld.this.chunkProvider.makeString();
            }
        });
    ...
    public boolean isSideSolid(BlockPos pos, EnumFacing side, boolean _default) {
        ...
        WrapperChunk wrapperChunk = getChunkFromBlockCoords(pos);
        if (wrapperChunk == null || wrapperChunk.isEmpty()) return _default;
    ...
    public Iterator<WrapperChunk> getPersistentChunkIterable(Iterator<WrapperChunk> chunkIterator) {
    ...
    public void markTileEntitiesInChunkForRemoval(WrapperChunk wrapperChunk) {
        for (TileEntity tileentity : wrapperChunk.getTileEntityMap()
            .values()) {
