...
public class BlockStateContainer implements IWrapperBlockStatePaletteResizer {

    private static final IWrapperBlockStatePalette REGISTRY_BASED_PALETTE = new BlockStatePaletteRegistry();
    protected static final IWrapperBlockState AIR_BLOCK_STATE = Blocks.AIR.getDefaultState();
    protected BitArray storage;
    protected IWrapperBlockStatePalette palette;
    private int bits;
    ...
    public int onResize(int bits, IWrapperBlockState state) {
        BitArray bitarray = this.storage;
        IWrapperBlockStatePalette iblockstatepalette = this.palette;
        this.setBits(bits);

        for (int i = 0; i < bitarray.size(); ++i) {
            IWrapperBlockState iblockstate = iblockstatepalette.getBlockState(bitarray.getAt(i));
    ...
    public void set(int x, int y, int z, IWrapperBlockState state) {
        this.set(getIndex(x, y, z), state);
    }

    protected void set(int index, IWrapperBlockState state) {
        int i = this.palette.idFor(state);
        this.storage.setAt(index, i);
    }

    public IWrapperBlockState get(int x, int y, int z) {
        return this.get(getIndex(x, y, z));
    }

    protected IWrapperBlockState get(int index) {
        IWrapperBlockState iblockstate = this.palette.getBlockState(this.storage.getAt(index));
