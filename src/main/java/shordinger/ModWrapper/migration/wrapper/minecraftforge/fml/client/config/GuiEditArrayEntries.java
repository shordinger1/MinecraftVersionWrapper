...
package shordinger.ModWrapper.migration.wrapper.minecraftforge.fml.client.config;
...
public class GuiEditArrayEntries extends GuiListExtended {
    ...
    public GuiEditArrayEntries(GuiEditArray parent, Minecraft mc, IConfigElement configElement, Object[] beforeValues,
        ...
        if (configElement.isList() && configElement.getArrayEntryClass() != null) {
            ...
            for (Object value : currentValues) listEntries
    ...
    public void recalculateState() {
        ...
        if (isDefault) for (int i = 0; i < listLength; i++) if (!configElement.getDefaults()[i].equals(
            listEntries.get(i)
                .getValue()))
            isDefault = false;

        if (!isChanged) for (int i = 0; i < listLength; i++) if (!beforeValues[i].equals(
            listEntries.get(i)
                .getValue()))
            isChanged = true;
    }
    ...
    protected void saveListChanges() {
        ...
        if (owningGui.slotIndex != -1 && owningGui.parentScreen != null
            ...
            for (int i = 0; i < listLength; i++) ao[i] = listEntries.get(i)
                .getValue();
            ...
            if (configElement.isList() && configElement.getType() == ConfigGuiType.BOOLEAN) {
                ...
                for (int i = 0; i < listLength; i++) abol[i] = Boolean.valueOf(
                    listEntries.get(i)
                        .getValue()
                        .toString());
                ...
                for (int i = 0; i < listLength; i++) ai[i] = Integer.valueOf(
                    listEntries.get(i)
                        .getValue()
                        .toString());
                ...
                for (int i = 0; i < listLength; i++) ad[i] = Double.valueOf(
                    listEntries.get(i)
                        .getValue()
                        .toString());
                ...
                for (int i = 0; i < listLength; i++) as[i] = listEntries.get(i)
                    .getValue()
                    .toString();
